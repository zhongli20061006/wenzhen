...
| 认证 | JWT (HS256) + bcrypt |
| AI 引擎 | **DeepSeek API**（症状提取 + 追问决策 + 终局综合） |
...
患者描述症状（自由文本）
    ↓
DeepSeek AI 一次性：提取症状 + 候选疾病 + 第一个追问（~2s）
    ↓
追问循环（最多5轮，每轮~2-4s）:
  1. 患者回答 YES / NO / UNKNOWN
  2. DeepSeek 基于全部累积上下文，选择下一个鉴别力最强的追问
  3. 增量收窄候选疾病（不重算）
    ↓
DeepSeek 综合：全部症状 + 对话记录 + 病史 → 推荐（科室 / 疾病 / 紧急性）
    ↓
患者选择科室/医生/时段 → 挂号
    ↓
医生查看报告 → 反馈诊断结果
...
> **架构特点**：DeepSeek 全权负责症状提取、追问决策、终局推荐。不再依赖 BERT NER 或关键词匹配。规则引擎仅在 DeepSeek 不可用时降级使用。
...
- **DeepSeek 全权驱动**：症状提取、追问决策、终局推荐全部由 DeepSeek 处理。知识库（科室/疾病/症状）作为 prompt 上下文注入，不做规则引擎干预。
- **线性增量流程**：一次提取 → 逐轮追问 → 终局综合。每轮仅发送当前累积状态给 DeepSeek，不做全量重算。
- **降级保底**：DeepSeek 不可用时自动回退到原有的规则引擎流程（NER + 关键词 + 评分系统），保证系统可用性。
