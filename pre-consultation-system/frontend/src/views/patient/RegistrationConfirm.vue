<template>
  <div class="reg-page">
    <div class="nav-bar">
      <el-button text @click="$router.back()"><ArrowLeft />返回</el-button>
      <el-button text @click="$router.push({ name: 'Profile' })"><User />个人中心</el-button>
    </div>
    <el-card>
      <template #header><h2><DocumentChecked style="margin-right:6px" />挂号确认</h2></template>
      <el-form :model="form" label-width="100px" v-loading="loadingDepts">
        ...
        <el-form-item>
          <el-button type="primary" @click="submit" :loading="submitting" style="width:100%">
            <Check style="margin-right:4px" />确认挂号
          </el-button>
        </el-form-item>
      </el-form>
    </el-card>
...
@media (max-width: 768px) {
  .reg-page { padding: 0 8px; margin: 6px auto; }
  .nav-bar { flex-wrap: wrap; gap: 4px; }
  .reg-page :deep(.el-form-item) { flex-wrap: wrap; }
  .reg-page :deep(.el-form-item__label) { float: none; display: block; text-align: left; padding-bottom: 4px; }
  .reg-page :deep(.el-form-item__content) { margin-left: 0 !important; width: 100%; }
  .reg-page :deep(.el-radio-group) { width: 100%; }
  .reg-page :deep(.el-radio) { flex: 1; min-width: 45%; margin-right: 0; min-height: 44px; display: flex; align-items: center; }
  .reg-page :deep(.el-select) { width: 100% !important; }
  .reg-page :deep(.el-date-editor) { width: 100% !important; }
  .reg-page .el-button--primary { min-height: 44px; }
}
</style>
